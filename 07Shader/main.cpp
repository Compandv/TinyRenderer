...
struct ToonShader : public IShader {
    ...
    virtual Vec4f vertex(int iface, int nthvert) {
        Vec4f gl_Vertex = embed<4>(model->vert(iface, nthvert)); //3 -> 4
        gl_Vertex = Projection * ModelView * gl_Vertex;
        varying_tri.set_col(nthvert, proj<3>(gl_Vertex / gl_Vertex[3]));//4 -> 3
    ...
    //bar 质心坐标  
    virtual bool fragment(Vec3f bar, TGAColor& color) {
        float intensity = varying_ity * bar; //向量点乘
...
struct PhongShader : public IShader {
    ...
    virtual bool fragment(Vec3f bar, TGAColor& color) {
        ...
        color = c;
        //照明模型有三个分量
        //1.环境照明（这个场景内是恒定的）  2.漫反射照明（法线、光源有关） 
        //3. 镜面反射光（类似局部高光，和反射光方向r和视图方向v有关）
