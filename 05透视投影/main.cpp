...
//绘制三角形(顶点坐标，uv坐标，tga指针，亮度，zbuffer)
void triangle1(Vec3i t0, Vec3i t1, Vec3i t2, Vec2i uv0, Vec2i uv1, Vec2i uv2, TGAImage& image, float intensity, int* zbuffer) {
	if (t0.y == t1.y && t0.y == t2.y) return;
	//分割成两个三角形
	if (t0.y > t1.y) { std::swap(t0, t1); std::swap(uv0, uv1); }
	if (t0.y > t2.y) { std::swap(t0, t2); std::swap(uv0, uv2); }
...
int main(int argc, char** argv) {
    ...
    {
        //初始化投影矩阵
        Matrix Projection = Matrix::identity(4); //四阶单位矩阵
        //初始化视角矩阵
        Matrix ViewPort   = viewport(width/8, height/8, width*3/4, height*3/4); //平移100， 放大600 
        ...
        for (int i=0; i<model->nfaces(); i++) {
            ...
            for (int j=0; j<3; j++) {
                ...
                //视角矩阵*投影矩阵*坐标
                screen_coords[j] = m2v(ViewPort * Projection * v2m(v));  // why？
                world_coords[j]  = v;
            }
            //计算法向量
            Vec3f n = (world_coords[2]-world_coords[0])^(world_coords[1]-world_coords[0]);// 相当于AC X AB
            n.normalize();
            //计算光照
